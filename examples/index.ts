import { Moderation } from '../src/index' // It need to be 'discord-moderation';
import { Client, Intents, MessageEmbed, TextChannel } from 'discord.js';
import ms from 'ms'; // If not installed, exec 'npm i ms @types/ms' into console.
...
client.on('message', (message) => {
    ...
    if(cmd === 'mute') {
        ...
        return moderation.mute(message.guild, member, (message.channel as TextChannel), reason);
    }
        ...
        return moderation.tempmute(message.guild, member, (message.channel as TextChannel), reason, Number(ms(time)));
    }
    else if(cmd === 'unmute') {
        const id = args[0];
        if(!id) return message.reply('write the id of the mute!');

        return moderation.unmute(message.guild, (message.channel as TextChannel), id);
    }
    else if(cmd === 'warn') {
        const member = message.mentions.members.first();
        var reason = args.slice(1).join(' ');
        ...
        return moderation.warn(member, (message.channel as TextChannel), reason);
    }
    else if(cmd === 'unwarn') {
        const member = message.mentions.members.first();
        if(!member) return message.reply('mention any member!');

        return moderation.unwarn(member, (message.channel as TextChannel));
    }
    else if(cmd === 'warns') {
        const member = message.mentions.members.first();
        if(!member) return message.reply('mention any member!');
